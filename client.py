...
def createTicket():
    ...
    mainLabel = Label(gameWindow, width = 65, height = 16, releif = 'ridge', bg = 'white', borderwidth=5)
    ...
    for row in range(0, 3):
        ...
        for col in range(0, 9):
            if (platform.system() == 'darwin'):
                boxButton = Button(gameWindow, font = ('Chalkboard SE', 18), borderwidth= 3, 
                                   padx= 22, pady = 23,bg = '#fff176', highlightbackground= '#fff176',
                                   activebackground= '#c5ela5')
                boxButton.place(x = xpos, y = ypos)
            else:
                boxButton = Tk.Button(gameWindow, font = ('Chalkboard SE', 30), width = 3, height = 3,
                                      borderwidth = 5, bg = '#fff176')
                boxButton.place(x = xpos, y = ypos)
...
def askPlayerName():
    ...
    button = Button(nameWindow, text="Save", font=("Chalkboard SE", 30),width=11, command=saveName, 
                    height=2, bg="#80deea", bd=3)
